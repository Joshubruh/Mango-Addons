...
def sell_offer_ms1(firstexec):
    ...
    coordinates.click(coordinates.second_ms)
...
def detect_outdated_ms1_sell():
    read_chat()
    if "OUT" in read_chat() or "ATED" in read_chat():
        print("Outdated: Relisting Order")
        handle_outdated_ms1_sell()
    elif "filled" in read_chat():
...
def sell_offer_serum(firstexec):
    print("Sell Offer Serum - EXECUTED")
    time.sleep(0.5)
    if firstexec == True:
        open_bazaar()
    else:
...
def create_buy_order_serum(firstexec):
    ...
    while True:
        detect_outdated_serum_buy()
        time.sleep(1)

def detect_outdated_serum_sell():
    read_chat()
    if "MATCHED" in read_chat() or "OUT" in read_chat() or "ATED" in read_chat():
        print("Outdated: Relisting Order")
        handle_outdated_serum_sell()
    elif "filled" in read_chat():
        handle_filled_serum_sell()
    elif "restart" in read_chat():
        warpout_failsafe("serum_sell")
    elif len(read_chat()) < 5:
        determine_connected("sell")
    elif "Escrow" in read_chat():
        sell_offer_serum(False)

def detect_outdated_serum_buy():
    if "MATCHED" in read_chat() or "OUT" in read_chat() or "ATED" in read_chat():
        handle_outdated_serum_buy()
    elif "filled" in read_chat():
        handle_filled_serum_buy()
    elif "restart" in read_chat():
...
def handle_outdated_serum_buy():
    print("[LOG]: Serum Buy Outdated")
    print("\a")
    cancel_order(False)
    time.sleep(1)
    if "Claim" not in read_chat():
        create_buy_order_serum(False)
    else:
        sell_offer_serum(False)

def handle_outdated_serum_sell():
